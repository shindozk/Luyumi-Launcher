...
export function setupWaylandEnvironment() {
  if (process.platform !== 'linux') {
    return {};
  }
  ...
  // envVars.ELECTRON_OZONE_PLATFORM_HINT = 'wayland'; // Not needed for Java game
...
function detectGpuLinux() {
  try {
    ...
    let integratedName: string | null = null;
    let dedicatedName: string | null = null;
    let hasNvidia = false;
    let hasAmd = false;
...
export function setupGpuEnvironment(gpuPreference: string = 'auto') {
  if (process.platform !== 'linux') {
    return {};
  }
