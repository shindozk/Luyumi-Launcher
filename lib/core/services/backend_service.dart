...
class BackendService {
  ...
  static const Duration launchTimeout = Duration(minutes: 5);
  ...
  static Future<Map<String, dynamic>> login(String name, String uuid) async {
    try {
      final response = await http
          .post(
            Uri.parse('$baseUrl/api/game-session/child'),
  ...
  static Future<List<dynamic>> getNews() async {
    try {
      Logger.info('BackendService.getNews() - Requesting $baseUrl/api/news');
      final response = await http
          .get(Uri.parse('$baseUrl/api/news'))
          .timeout(timeout);

      Logger.info('BackendService.getNews() - Status: ${response.statusCode}');
      Logger.info(
        'BackendService.getNews() - Response length: ${response.body.length}',
      );

      if (response.statusCode == 200) {
        final data = jsonDecode(response.body);
        Logger.info('BackendService.getNews() - Decoded ${data.length} items');
        return data;
      } else {
        Logger.error(
          'BackendService.getNews() - HTTP ${response.statusCode}: ${response.body}',
        );
      }
    } catch (e, stack) {
      Logger.error('BackendService.getNews() - Failed to fetch news: $e');
  ...
  static Future<Map<String, dynamic>> searchMods({
    ...
    try {
      ...
      Logger.error('Error searching mods: $e');
    }
    return {
      'data': [],
  ...
  static Future<Map<String, dynamic>?> getModDetails(int modId) async {
    try {
      final response = await http
          .get(Uri.parse('$baseUrl/api/mods/details/$modId'))
          .timeout(timeout);
      ...
      Logger.error('Error getting mod details: $e');
    }
    return null;
  }
  ...
  static Future<String> getModDescription(int modId) async {
    try {
      final response = await http
          .get(Uri.parse('$baseUrl/api/mods/description/$modId'))
          .timeout(timeout);
      ...
      Logger.error('Error getting mod description: $e');
    }
    return '';
  }
