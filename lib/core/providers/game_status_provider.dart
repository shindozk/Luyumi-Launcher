...
class GameStatusProvider extends ChangeNotifier {
  ...
  String _installStatus = 'idle';
  String _repairStatus = 'idle';
  ...
  Future<bool> startInstall({String? version}) async {
    ...
    try {
      ...
      if (response != null && response['success'] == true) {
        final success = await _waitForOperationCompletion(isInstall: true);
        ...
        throw Exception(_installMessage.isNotEmpty
            ? _installMessage
            : 'Installation failed');
      } else {
  ...
  Future<void> _pollProgress() async {
    ...
    try {
      ...
      if (response != null) {
        ...
        final status = response['status'] ?? 'idle';
  ...
  Future<bool> startRepair() async {
    ...
    try {
      ...
      if (response != null && response['success'] == true) {
        final success = await _waitForOperationCompletion(isInstall: false);
        _stopProgressPolling();
        if (success) {
        ...
        throw Exception(
            _repairMessage.isNotEmpty ? _repairMessage : 'Repair failed');
      } else {
        throw Exception(response?['message'] ?? 'Unknown error');
      }
    } catch (err) {
  ...
  Future<bool> _waitForOperationCompletion({required bool isInstall}) async {
    final start = DateTime.now();
    const timeout = Duration(hours: 2);

    while (true) {
      final status = isInstall ? _installStatus : _repairStatus;
      if (status == 'completed') {
        return true;
      }
      if (status == 'error') {
      ...
      if (DateTime.now().difference(start) > timeout) {
        if (isInstall) {
          _installMessage = 'Installation timeout';
        } else {
          _repairMessage = 'Repair timeout';
        }
        notifyListeners();
        return false;
      }
