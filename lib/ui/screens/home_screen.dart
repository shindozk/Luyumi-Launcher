...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void _stopUiTimer() {
    ...
    if (_gameStartTime != null) {
      final duration = DateTime.now().difference(_gameStartTime!);
      _saveLastSessionTime(duration.inSeconds);
    }
    if (mounted) {
      setState(() {
        _elapsedTime = "00:00";
  ...
  Future<void> _handlePlay() async {
    ...
    setState(() => _isLoading = true);
    try {
      final persistentUuid = await PlayerService.getOrCreatePlayerId();
      String? savedUuid = await _authService.getSavedUuid();
      final uuid = savedUuid ?? persistentUuid;
      await _authService.saveUuid(uuid);

      // 1. Authenticate
      AuthResponse authResponse;
      try {
        authResponse = await _authService.login(username, uuid: uuid);
        if (mounted) {
          setState(() {
            _username = username;
            _uuid = uuid;
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                tr('auth_failed_offline', namedArgs: {'error': e.toString()}),
              ), // Make sure to add this key or use a static string for now
              backgroundColor: Colors.orange,
              duration: const Duration(seconds: 3),
            ),
          );
        }
        // Create offline auth response
        authResponse = AuthResponse(
          identityToken: "",
          sessionToken: "",
          mode: "offline",
  ...
  Future<void> _loadLastSessionTime() async {
    final durationSeconds = await _settingsService.getLastSessionDuration();
    if (durationSeconds > 0) {
      ...
      setState(() {
        if (hours > 0) {
          _lastSessionTime = "${hours}h ${minutes}m";
        } else if (minutes > 0) {
          _lastSessionTime = "${minutes}m";
        } else {
          _lastSessionTime = "${durationSeconds}s";
        }
      });
    }
  }

  Future<void> _saveLastSessionTime(int seconds) async {
    if (seconds < 5) return; // Don't save if less than 5 seconds
    await _settingsService.setLastSessionDuration(seconds);
    await _loadLastSessionTime();
  }
          ...
          children: [
            ...
            FadeInEntry(
              ...
              child: GameActionPanel(
                ...
                lastSessionTime: _lastSessionTime,
